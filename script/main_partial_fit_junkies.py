...
if __name__ == '__main__':
    ...
    # vect = CountVectorizer(input="list")
    ...
    while(total_processed < total_word):
        ...
        # clf_2.partial_fit(vX[first_word:last_word], vy[first_word:last_word])
        # clf_2.partial_fit(X[:99, :], y[:99, 0], classes=[0, 1])

        # vX = vect.fit_transform(X[first_word:last_word])
        vX = vect.fit(X[first_word:last_word])
        # vy = vect.fit_transform(y[first_word:last_word])
        vy = vect.fit(y[first_word:last_word])
        # clf_2.partial_fit(vX, vy)
